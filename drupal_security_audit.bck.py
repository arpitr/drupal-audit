            ...
            try:
                ...
                for package, package_advisories in advisories.items():
                    for advisory in package_advisories:
                        ...
                        print(f"{Colors.WARNING}Title:{Colors.ENDC} {advisory.get('title', 'N/A')}")
                        print(f"{Colors.WARNING}Severity:{Colors.ENDC} {advisory.get('severity', 'N/A')}")
                        print(f"{Colors.WARNING}CVE:{Colors.ENDC} {advisory.get('cve', 'N/A')}")
                        print(f"{Colors.WARNING}Link:{Colors.ENDC} {advisory.get('link', 'N/A')}")
                        print("-" * 80)
                
                self.results['composer_audit']['status'] = 'failed'
                self.results['composer_audit']['vulnerabilities_count'] = vuln_count
                self.results['composer_audit']['details'] = advisories
                
                print(f"\n{Colors.FAIL}Total vulnerabilities found: {vuln_count}{Colors.ENDC}")
                return False
                
            except json.JSONDecodeError:
                print(f"{Colors.FAIL}✗ Error parsing composer audit output{Colors.ENDC}")
                print(f"STDOUT: {stdout}")
                print(f"STDERR: {stderr}")
                self.results['composer_audit']['status'] = 'error'
                self.results['composer_audit']['error'] = stderr
                return False
